...
bandMinInput.addEventListener('change', applyBandInputs);
bandMaxInput.addEventListener('change', applyBandInputs);
...
function updateFrequencyScale() {
  ...
  for (const mark of scaleTickMarks(bandMinHz, bandMaxHz)) {
    ...
    if (waterfallOrientation === 'vertical') tick.style.left = `${position}%`;
    else tick.style.top = `${100 - position}%`;
